...
<script setup>
import {ref, onMounted, shallowRef, provide, defineProps, defineEmits} from 'vue';
...
// 计算手机的宽度和高度
const screenHeight = window.innerHeight * 0.95; // 90% of the screen width
const screenWidth = screenHeight * (14/ 30); // 根据高宽比计算高度
...
function setRightPhotoContainer() {
  document.querySelectorAll('.logoRight').forEach(element => {
  //   获取元素高度
...
/* 如果使用了-webkit-line-clamp，可能还需要以下规则 */
...
.logo.left {
  display: inline-block; /* 适应ElDropdown的布局 */
  position: relative;
}

.logoRight {
  height: 90%; /* 容器高度为视口高度的80% */
